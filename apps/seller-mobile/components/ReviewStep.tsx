...
import { Check, ChevronRight, Pencil, Clock, ShieldCheck, Sparkles, Store, Package, Tag, User } from 'lucide-react-native'
...
export default function ReviewStep({ onEditStore, onEditBusiness, onEditBank, onBack, onApproved }: Props) {
  ...
  if (submitState === 'review' || submitState === 'approved') {
    return <StatusScreen state={submitState} reduced={reduced} onContinue={handleGoToDashboard} t={t} sellerName={seller.name} />
  }
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={styles.sections}>
          ...
          >
            ...
            {draft.description ? <Row label={t('seller.setup.rowDescription')} value={draft.description} /> : null}
      ...
      <View style={[styles.ctaDock, { paddingBottom: insets.bottom + spacing[4] }]}>
        <TouchableOpacity
          ...
          accessibilityLabel={submitState === 'submitting' ? t('seller.setup.submitted') : t('seller.setup.submit')}
        >
...
function StatusScreen({ state, reduced, onContinue, t, sellerName }: {
  state: 'review' | 'approved'; reduced: boolean; onContinue: () => void; t: any; sellerName: string
}) {
  const insets = useSafeAreaInsets()
  ...
  return (
    <View style={[styles.statusContainer, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      <Animated.View
        entering={reduced ? FadeIn : SlideInUp.springify().damping(20).stiffness(300)}
        style={styles.statusBody}
      >
        <View style={[styles.statusIconWrap, isApproved ? styles.statusIconApproved : styles.statusIconReview]}>
          {isApproved ? (
            <Animated.View style={checkStyle}>
              <Check size={36} color={colors.success} strokeWidth={3} />
            </Animated.View>
          ) : (
            <Clock size={36} color="#F59E0B" strokeWidth={2} />
          )}
        </View>

        <View style={[styles.statusPill, isApproved ? styles.statusPillApproved : styles.statusPillReview]}>
          <Text style={[styles.statusPillText, isApproved ? styles.statusPillTextApproved : styles.statusPillTextReview]}>
...
function ReviewSection({ title, editAria, onEdit, t, children }: {
  title: string; editAria: string; onEdit: () => void; t: any; children: React.ReactNode
}) {
  return (
    <View style={styles.sectionCard} accessibilityRole="summary">
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle} accessibilityRole="header">{title}</Text>
        <TouchableOpacity onPress={onEdit} accessibilityRole="button" accessibilityLabel={editAria} style={styles.editLink}>
...
const styles = StyleSheet.create({
  ...
  sectionCard: { backgroundColor: colors.surface, borderRadius: radii.lg, borderWidth: 1, borderColor: colors.borderLight, padding: spacing[4] },
  ...
  spinner: { width: 18, height: 18, borderRadius: 9, borderWidth: 2, borderColor: 'rgba(255,255,255,0.3)', borderTopColor: colors.white },
  ...
  statusIconWrap: { width: 72, height: 72, borderRadius: 36, alignItems: 'center', justifyContent: 'center', marginBottom: spacing[1] },
  statusIconReview: { backgroundColor: '#FEF3C7' },
  ...
  statusPillApproved: { backgroundColor: '#DCFCE7' },
  statusPillText: { fontSize: 12, fontWeight: '600' },
  statusPillTextReview: { color: '#92400E' },
  statusPillTextApproved: { color: '#166534' },
  statusTitle: { fontSize: 24, fontWeight: '700', color: colors.text, textAlign: 'center', fontFamily: fontFamily.sansBold[0] },
  statusSubtitle: { fontSize: 15, color: colors.textMuted, textAlign: 'center', lineHeight: 22 },
  timeframePill: { flexDirection: 'row', alignItems: 'center', gap: spacing[1.5], backgroundColor: colors.surface, borderRadius: radii.full, paddingHorizontal: spacing[3], paddingVertical: spacing[1.5], borderWidth: 1, borderColor: colors.borderLight },
  timeframeText: { fontSize: 13, fontWeight: '600', color: '#92400E' },
  statusDesc: { fontSize: 14, color: colors.textMuted, textAlign: 'center', lineHeight: 20, maxWidth: 320 },
})
