...
function Field({ label, hint, error, errorId: _errorId, children }: { label: string; hint?: string; error?: string; errorId?: string; children: React.ReactNode }) {
  return (
...
export function MediaSection({ form, errors, updateField, t, reduced: _reduced }: { form: FormState; errors: Record<string, string>; updateField: (f: keyof FormState, v: string | string[]) => void; t: (k: string, o?: Record<string, unknown>) => string; reduced: boolean }) {
...
export function DetailsSection({ form, errors, updateField, t, categories: _categories, categoryTree, reduced: _reduced }: { form: FormState; errors: Record<string, string>; updateField: (f: keyof FormState, v: string | string[]) => void; t: (k: string, o?: Record<string, unknown>) => string; categories?: { id: string; name: string }[]; categoryTree?: Category[]; reduced: boolean }) {
  ...
  return (
    <View style={styles.sectionContent}>
      ...
      <Field label={t('seller.products.formFieldTags')} hint={t('seller.products.formFieldTagsHint')}>
        ...
        {form.tags.length >= 10 && (
          <Text style={styles.tagMaxWarn}>{t('seller.products.formTagsMaxWarn')}</Text>
        )}
      </Field>
      ...
      <BottomSheet visible={catPickerOpen} onClose={() => setCatPickerOpen(false)} title={t('seller.products.formFieldCategory')}>
        ...
        <ScrollView style={styles.catList} keyboardShouldPersistTaps="handled">
          ...
          {filteredCats.length === 0 && (
            <Text style={styles.catEmpty}>{t('seller.products.formNoCategories')}</Text>
          )}
        </ScrollView>
      </BottomSheet>
    </View>
...
export function PricingSection({ form, errors, updateField, t, reduced: _reduced }: { form: FormState; errors: Record<string, string>; updateField: (f: keyof FormState, v: string | string[] | VariantOption[] | VariantRow[]) => void; t: (k: string, o?: Record<string, unknown>) => string; reduced: boolean }) {
...
export function DescriptionSection({ form, errors, updateField, t, reduced: _reduced }: { form: FormState; errors: Record<string, string>; updateField: (f: keyof FormState, v: string | string[] | VariantOption[] | VariantRow[] | SpecRow[]) => void; t: (k: string, o?: Record<string, unknown>) => string; reduced: boolean }) {
