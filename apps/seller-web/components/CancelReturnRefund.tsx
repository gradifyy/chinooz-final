...
> = {
  none: { bg: 'bg-gray-100', text: 'text-gray-600', dot: 'bg-gray-400', labelKey: 'seller.orders.refundNone', icon: Clock },
  pending: { bg: 'bg-warning/10', text: 'text-warning', dot: 'bg-warning', labelKey: 'seller.orders.refundPending', icon: Clock },
  refunded: { bg: 'bg-success/10', text: 'text-success', dot: 'bg-success', labelKey: 'seller.orders.refundRefunded', icon: Check },
  rejected: { bg: 'bg-error/10', text: 'text-error', dot: 'bg-error', labelKey: 'seller.orders.refundRejected', icon: XCircle },
}
...
> = {
  requested: { bg: 'bg-warning/10', text: 'text-warning', dot: 'bg-warning', labelKey: 'seller.orders.returnRequested' },
  approved: { bg: 'bg-info/10', text: 'text-info', dot: 'bg-info', labelKey: 'seller.orders.returnApproved' },
  rejected: { bg: 'bg-error/10', text: 'text-error', dot: 'bg-error', labelKey: 'seller.orders.returnRejected' },
  refund_processed: { bg: 'bg-success/10', text: 'text-success', dot: 'bg-success', labelKey: 'seller.orders.returnRefundProcessed' },
}
...
export function RefundBreakdownTable({
  ...
  return (
    <div className="bg-surface rounded-xl border border-border-light p-4 space-y-2" aria-labelledby="refund-breakdown-heading">
      ...
      <div className="flex items-center justify-between py-0.5">
        ...
        <span className="text-sm text-text tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.refundSubtotal')}: ${formatNPR(breakdown.subtotal)}`}>
          {formatNPR(breakdown.subtotal)}
        </span>
      </div>
      <div className="flex items-center justify-between py-0.5">
        <span className="text-sm text-text-muted">{t('seller.orders.refundVatIncl')}</span>
        <span className="text-sm text-text-muted tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.refundVatIncl')}: ${formatNPR(breakdown.vatInclusive)}`}>
          {formatNPR(breakdown.vatInclusive)}
        </span>
      </div>
      <div className="flex items-center justify-between py-0.5">
        <span className="text-sm text-text-muted">{t('seller.orders.refundDeliveryFee')}</span>
        <span className="text-sm text-text tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.refundDeliveryFee')}: ${formatNPR(breakdown.deliveryFee)}`}>
          {formatNPR(breakdown.deliveryFee)}
        </span>
      </div>
      {breakdown.discount > 0 && (
        <div className="flex items-center justify-between py-0.5">
          <span className="text-sm text-text-muted">{t('seller.orders.refundDiscount')}</span>
          <span className="text-sm text-text-muted tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.refundDiscount')}: ${formatNPR(breakdown.discount)}`}>
            -{formatNPR(breakdown.discount)}
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-base font-bold text-text tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.refundTotal')}: ${formatNPR(breakdown.refundTotal)}`}>
          {formatNPR(breakdown.refundTotal)}
        </span>
      </div>
...
export function CancelOrderSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="cancel-sheet-title">
      ...
      >
        <div className="sticky top-0 bg-surface border-b border-border-light px-5 py-4 flex items-center justify-between">
          <h3 id="cancel-sheet-title" className="text-lg font-semibold text-text flex items-center gap-2">
            <AlertTriangle size={18} className="text-error" aria-hidden="true" />
            {t('seller.orders.cancelTitle')}
          </h3>
          <button onClick={() => !busy && onClose()} className="h-8 w-8 flex items-center justify-center rounded-full hover:bg-background" aria-label={t('seller.orders.cancelClose')}>
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="rounded-lg bg-background border border-border-light p-3 space-y-1.5">
            <p className="text-sm font-semibold text-text" style={{ fontFamily: 'ui-monospace, monospace' }}>
              {order.orderId}
            </p>
            <p className="text-sm text-text-muted">{order.buyerName} · {order.itemCount} {order.itemCount === 1 ? t('seller.orders.item') : t('seller.orders.items')}</p>
          ...
          <div className="space-y-2">
            <p className="text-sm font-semibold text-text" id="cancel-reason-group-label">{t('seller.orders.cancelReasonLabel')}</p>
            <div role="radiogroup" aria-labelledby="cancel-reason-group-label" className="space-y-1">
              {CANCEL_REASONS.map(r => (
                <label key={r.key} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-background cursor-pointer border border-transparent has-[:checked]:border-primary/30 has-[:checked]:bg-primary/5">
                  <input
                    type="radio"
                    name="cancel-reason"
                    value={r.key}
          ...
          {isPrepaid && (
            <div className="rounded-lg border border-border-light p-3 space-y-1.5">
              <p className="text-[12px] font-semibold text-text-muted uppercase tracking-wide">{t('seller.orders.refundBreakdownTitle')}</p>
              <div className="flex justify-between text-sm">
                <span className="text-text-muted">{t('seller.orders.refundSubtotal')}</span>
                <span className="text-text tabular-nums" style={TABNUM}>{formatNPR(breakdown.subtotal)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-muted">{t('seller.orders.refundVatIncl')}</span>
                <span className="text-text-muted tabular-nums" style={TABNUM}>{formatNPR(breakdown.vatInclusive)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-muted">{t('seller.orders.refundDeliveryFee')}</span>
                <span className="text-text tabular-nums" style={TABNUM}>{formatNPR(breakdown.deliveryFee)}</span>
              </div>
              <div className="h-px bg-border-light my-1" />
              <div className="flex justify-between">
                <span className="text-sm font-bold text-text">{t('seller.orders.refundTotal')}</span>
                <span className="text-sm font-bold text-text tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.refundTotal')}: ${formatNPR(breakdown.refundTotal)}`}>{formatNPR(breakdown.refundTotal)}</span>
              </div>
            </div>
          )}
        ...
        <div className="sticky bottom-0 bg-surface border-t border-border-light px-5 py-3 flex gap-3">
          ...
          <button
            ...
            aria-label={t('seller.orders.cancelConfirmAria', { id: order.orderId, amount: formatNPR(order.total) })}
          >
    ...
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[12px] font-semibold shrink-0 ${statusMeta.bg} ${statusMeta.text}`}>
        ...
        <div className="space-y-1">
          <p className="text-[12px] font-semibold text-text-muted uppercase tracking-wide">{t('seller.orders.returnReasonLabel')}</p>
          <p className="text-sm text-text">{t(`seller.orders.cancelReason${request.reason.charAt(0).toUpperCase()}${request.reason.slice(1)}`)}</p>
        ...
        <div className="space-y-1.5">
          <p className="text-[12px] font-semibold text-text-muted uppercase tracking-wide">{t('seller.orders.returnItemsLabel')}</p>
          {request.items.map(item => (
            <div key={item.itemId} className="flex items-center gap-2.5">
              <SafeImage src={item.image} alt="" className="w-10 h-10 rounded-md object-cover bg-shimmer shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm text-text truncate">{item.name.split('—')[0]?.trim() || item.name}</p>
        ...
        {request.photoUrls.length > 0 && (
          <div className="space-y-1">
            <p className="text-[12px] font-semibold text-text-muted uppercase tracking-wide">{t('seller.orders.returnPhotosLabel')}</p>
        ...
        <div className="flex items-center justify-between rounded-lg bg-background border border-border-light px-3 py-2">
          <span className="text-sm font-semibold text-text-muted">{t('seller.orders.returnRequestedAmount')}</span>
          <span className="text-base font-bold text-text tabular-nums" style={TABNUM} aria-label={`${t('seller.orders.returnRequestedAmount')}: ${formatNPR(request.requestedAmount)}`}>
            {formatNPR(request.requestedAmount)}
          </span>
        </div>
        ...
        {isResolved && request.resolutionNote && (
          <div className="rounded-lg bg-background border border-border-light p-2.5">
            <p className="text-[12px] font-semibold text-text-muted uppercase tracking-wide mb-1">{t('seller.orders.returnResolutionNote')}</p>
        ...
        {request.refundStatus !== 'none' && (
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold text-text-muted">{t('seller.orders.refundStatusLabel')}</span>
        ...
        {!isResolved && (
          <div className="flex items-center gap-2 pt-1">
            <motion.button
              ...
              aria-label={t('seller.orders.returnApproveAria', { id: request.orderId, amount: formatNPR(request.requestedAmount) })}
            >
              <Check size={15} />
...
function ReturnResolutionSheet({
  ...
  const titleKey = isApprove ? 'seller.orders.returnApproveTitle' : 'seller.orders.returnRejectTitle'
  const subtitleKey = isApprove ? 'seller.orders.returnApproveSubtitle' : 'seller.orders.returnRejectSubtitle'
  const confirmKey = isApprove ? 'seller.orders.returnApproveConfirm' : 'seller.orders.returnRejectConfirm'
  const confirmAriaKey = isApprove ? 'seller.orders.returnApproveConfirmAria' : 'seller.orders.returnRejectConfirmAria'
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="return-resolution-title">
      <div className="absolute inset-0 bg-black/50" onClick={() => !busy && onClose()} />
      <motion.div
        ref={containerRef}
        initial={reduced ? false : { opacity: 0, y: 24, scale: 0.97 }}
      ...
      >
        <div className="sticky top-0 bg-surface border-b border-border-light px-5 py-4 flex items-center justify-between">
          <h3 id="return-resolution-title" className="text-lg font-semibold text-text flex items-center gap-2">
            {isApprove ? (
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="rounded-lg bg-background border border-border-light p-3 space-y-1.5">
            ...
            <p className="text-sm text-text-muted">
              {request.buyerName} · {request.items.length} {request.items.length === 1 ? t('seller.orders.item') : t('seller.orders.items')}
            </p>
            <p className="text-sm text-text">
              {t('seller.orders.returnAmountToRefund')}: <span className="font-semibold tabular-nums" style={TABNUM}>{formatNPR(request.requestedAmount)}</span>
            </p>
            {isApprove && (
              <p className="text-[12px] text-text-muted pt-1">{t('seller.orders.returnApproveRestockNote')}</p>
            )}
          </div>

          <div>
            <label htmlFor="resolution-note" className="block text-sm font-semibold text-text mb-1.5">
        ...
        <div className="sticky bottom-0 bg-surface border-t border-border-light px-5 py-3 flex gap-3">
          ...
          <button
            ...
            style={isApprove ? undefined : { backgroundColor: '#DC2626' }}
            aria-label={t(confirmAriaKey, { id: request.orderId, amount: formatNPR(request.requestedAmount) })}
          >
            {busy ? <Loader2 size={16} className="animate-spin" /> : isApprove ? <Check size={16} /> : <XCircle size={16} />}
            {t(confirmKey)}
          </button>
        </div>
      </motion.div>
    ...
    <div className="space-y-4">
      ...
      {requests.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-base font-semibold text-text">{t('seller.orders.returnEmptyTitle')}</h3>
          <p className="text-sm text-text-muted mt-1 max-w-xs">{t('seller.orders.returnEmptySubtitle')}</p>
        </div>
      )}
    </div>
  )
