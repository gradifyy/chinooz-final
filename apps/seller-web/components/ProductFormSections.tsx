...
export type SectionKey = 'media' | 'details' | 'pricing' | 'description'
...
export function MediaSection({
  ...
  updateField: (f: keyof FormState, v: string | string[]) => void
  ...
  const retryUpload = (id: string) => {
    ...
    if (!img) return
    setMediaImages(prev =>
      prev.map(i => (i.id === id ? { ...i, error: false, uploading: true, progress: 0 } : i)),
    )
    let pct = 0
    const interval = setInterval(() => {
      pct += Math.random() * 30 + 15
      if (pct >= 100) {
        ...
        setMediaImages(prev => {
          const next = prev.map(i =>
            i.id === id ? { ...i, uploading: false, progress: 100, error: false } : i,
          )
          syncToForm(next)
          return next
        })
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        <div
          ...
          role="group"
          aria-label={t('seller.products.formFieldImage')}
        >
          {mediaImages.map((img, idx) => {
            ...
            return (
              <div
                ...
                aria-label={t('seller.products.mediaThumbAria', {
                  n: idx + 1,
                  cover: isCover ? `, ${t('seller.products.mediaCoverLabel')}` : '',
                })}
              >
                {/* Image */}
                {img.error ? (
                  ...
                  <div className="w-full h-full bg-shimmer flex items-center justify-center">
                    <div className="absolute bottom-0 left-0 right-0 h-1 bg-border">
                      <div
                        ...
                        aria-valuemin={0}
                        aria-valuemax={100}
                        aria-label={t('seller.products.mediaUploadProgress', {
                          percent: Math.round(img.progress),
                        })}
                      />
                    </div>
                  </div>
        ...
        {mediaImages.length >= MAX_IMAGES && (
          <p className="text-[12px] text-warning">
            {t('seller.products.mediaMaxCount', { max: MAX_IMAGES })}
          </p>
        )}
      ...
      {/* Video URL */}
      <Field
        label={t('seller.products.formFieldVideoUrl')}
        hint={t('seller.products.formFieldVideoUrlHint')}
      >
...
function generateSkuFromName(name: string): string {
  ...
  const prefix = words
    .map(w => w.replace(/[^a-z0-9]/g, '').slice(0, 4))
    .join('-')
    .toUpperCase()
...
export function DetailsSection({
  ...
  const selectedPath = useMemo(
    () => findCategoryPath(categoryTree, form.categoryId),
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Product name */}
      <Field
        label={t('seller.products.formFieldName')}
        hint={t('seller.products.formFieldNameHint')}
        error={errors.name}
        errorId="error-name"
      >
        <input
      ...
      {/* Brand */}
      <Field
        label={t('seller.products.formFieldBrand')}
        hint={t('seller.products.formFieldBrandHint')}
      >
        <input
          type="text"
          value={form.brand}
      ...
      {/* SKU + auto-suggest */}
      <Field
        label={t('seller.products.formFieldSku')}
        hint={t('seller.products.formFieldSkuHint')}
        error={errors.sku}
        errorId="error-sku"
      >
      ...
      <Field
        label={t('seller.products.formFieldCategory')}
        error={errors.categoryId}
        errorId="error-categoryId"
      >
        <div ref={catRef} className="relative">
          <button
            type="button"
            onClick={() => {
              setCatPickerOpen(o => !o)
              setCatSearch('')
            }}
              ...
              >
                ...
                <div className="overflow-y-auto flex-1">
                  ...
                  {filteredCats.length === 0 && (
                    <p className="px-3 py-4 text-[13px] text-text-muted text-center">
                      No categories found
                    </p>
                  )}
                </div>
      ...
      <Field
        label={t('seller.products.formFieldTags')}
        hint={t('seller.products.formFieldTagsHint')}
      >
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-surface p-2 min-h-[44px] focus-within:border-primary transition-colors">
          ...
          <input
            ...
            placeholder={
              form.tags.length === 0 ? t('seller.products.formFieldTagsPlaceholder') : ''
            }
        ...
        {form.tags.length >= 10 && <p className="text-[12px] text-warning mt-1">Maximum 10 tags</p>}
      </Field>
...
export function PricingSection({
  ...
  form: FormState
  errors: Record<string, string>
  updateField: (f: keyof FormState, v: string | string[] | VariantOption[] | VariantRow[]) => void
  t: (k: string, o?: Record<string, unknown>) => string
  reduced: boolean
}) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!hasOptions && (
        <>
          <div className="grid sm:grid-cols-2 gap-4">
            <Field
              label={t('seller.products.formFieldPrice')}
              hint={t('seller.products.formFieldPriceHint')}
              error={errors.price}
              errorId="error-price"
            >
              <input
              ...
              {priceNum > 0 && (
                <p className="text-[12px] text-text-muted mt-1 tabular-nums">
                  {t('seller.products.vatInclusive')} · NPR {vatAmt.toLocaleString('en-IN')}
                </p>
              )}
            </Field>
            <Field
              label={t('seller.products.formFieldCompareAtPrice')}
              hint={t('seller.products.formFieldCompareAtPriceHint')}
              error={errors.compareAtPrice}
              errorId="error-compareAtPrice"
            >
              <input
                type="number"
                inputMode="numeric"
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Field
              label={t('seller.products.formFieldStock')}
              hint={t('seller.products.formFieldStockHint')}
              error={errors.stockCount}
              errorId="error-stockCount"
            >
              <input
                type="number"
                inputMode="numeric"
            ...
            <Field
              label={t('seller.products.formFieldCostPrice')}
              hint={t('seller.products.formFieldCostPriceHint')}
            >
              <input
                type="number"
                inputMode="numeric"
          ...
          {margin !== null && (
            <p
              className={`text-[12px] font-medium ${margin < 0 ? 'text-error' : margin < 20 ? 'text-warning' : 'text-success'}`}
            >
              {margin < 0
      ...
      <div className="flex flex-col gap-3">
        <div>
          <h4 className="text-[14px] font-semibold text-text">
            {t('seller.products.variantsTitle')}
          </h4>
          <p className="text-[12px] text-text-muted mt-0.5">
            {t('seller.products.variantsSubtitle')}
          </p>
        </div>
      ...
      {hasOptions && variants.length > 0 && (
        <div className="flex flex-col gap-3">
          <h4 className="text-[14px] font-semibold text-text">
            {t('seller.products.variantsMatrixTitle')}
          </h4>

          {/* Bulk-edit row */}
          <div className="flex items-center gap-2 rounded-md border border-border-light bg-background px-3 py-2">
            <span className="text-[12px] font-semibold text-text-muted shrink-0">
              {t('seller.products.variantsBulkApply')}:
            </span>
            <input
              type="number"
              inputMode="numeric"
            ...
            <button
              ...
              disabled={!bulkPrice}
              className="h-8 px-2 rounded-md bg-primary text-white text-[12px] font-semibold disabled:opacity-50"
            >
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-border bg-background sticky top-0">
                  <th
                    scope="col"
                    className="px-3 py-2 text-left text-[12px] font-semibold text-text-muted uppercase"
                  >
                    {t('seller.products.variantsColVariant')}
                  </th>
                  <th
                    scope="col"
                    className="px-3 py-2 text-left text-[12px] font-semibold text-text-muted uppercase"
                  >
                    {t('seller.products.variantsColSku')}
                  </th>
                  <th
                    scope="col"
                    className="px-3 py-2 text-right text-[12px] font-semibold text-text-muted uppercase"
                  >
                    {t('seller.products.variantsColPrice')}
                  </th>
                  <th
                    scope="col"
                    className="px-3 py-2 text-right text-[12px] font-semibold text-text-muted uppercase"
                  >
                    {t('seller.products.variantsColCompareAt')}
                  </th>
                  <th
                    scope="col"
                    className="px-3 py-2 text-right text-[12px] font-semibold text-text-muted uppercase"
                  >
                    {t('seller.products.variantsColStock')}
                  </th>
                  <th
                    scope="col"
                    className="px-3 py-2 text-center text-[12px] font-semibold text-text-muted uppercase"
                  >
                    {t('seller.products.variantsColEnabled')}
                  </th>
                </tr>
              </thead>
              <tbody>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5">
                          ...
                          {v.price && Number(v.price) > 0 && (
                            <p className="text-[10px] text-text-muted mt-0.5 tabular-nums">
                              {t('seller.products.vatInclusive')}
                            </p>
                          )}
                        </td>
                        <td className="px-3 py-2.5">
                          <input
                            ...
                            placeholder="0"
                            aria-label={t('seller.products.variantsCompareAtAria', {
                              label: v.label,
                            })}
                          ...
                          {vErrs.compareAt && (
                            <p className="text-[10px] text-error mt-0.5">
                              {t('seller.products.variantsErrorCompareAt')}
                            </p>
                          )}
                        </td>
                        <td className="px-3 py-2.5">
                        ...
                        <td className="px-3 py-2.5 text-center">
                          ...
                          >
                            <span
                              className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${v.enabled ? 'translate-x-4' : 'translate-x-0.5'}`}
                            />
                          </button>
                        </td>
                      </motion.tr>
...
export function DescriptionSection({
  ...
  updateField: (
    f: keyof FormState,
    v: string | string[] | VariantOption[] | VariantRow[] | SpecRow[],
  ) => void
  t: (k: string, o?: Record<string, unknown>) => string
  reduced: boolean
}) {
  ...
  const updateSpecRow = (id: string, field: 'key' | 'value', value: string) => {
    updateField(
      'specRows',
      form.specRows.map(r => (r.id === id ? { ...r, [field]: value } : r)),
    )
  }
  ...
  const removeSpecRow = (id: string) => {
    updateField(
      'specRows',
      form.specRows.filter(r => r.id !== id),
    )
  }
  ...
  const toolbarBtns = [
    {
      key: 'bold',
      label: t('seller.products.editorBold'),
      aria: t('seller.products.editorBoldAria'),
      icon: 'B',
      action: () => insertMarkdown('**', '**'),
    },
    {
      key: 'italic',
      label: t('seller.products.editorItalic'),
      aria: t('seller.products.editorItalicAria'),
      icon: 'I',
      action: () => insertMarkdown('*', '*'),
    },
    {
      key: 'heading',
      label: t('seller.products.editorHeading'),
      aria: t('seller.products.editorHeadingAria'),
      icon: 'H',
      action: () => insertMarkdown('## '),
    },
    {
      key: 'bullet',
      label: t('seller.products.editorBulletList'),
      aria: t('seller.products.editorBulletListAria'),
      icon: '•',
      action: () => insertMarkdown('- '),
    },
  ]
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field
        label={
          descLang === 'en'
            ? t('seller.products.formFieldDescription')
            : t('seller.products.formFieldDescriptionNe')
        }
        hint={
          descLang === 'en'
            ? t('seller.products.formFieldDescriptionHint')
            : t('seller.products.formFieldDescriptionNeHint')
        }
          ...
          {(['en', 'ne'] as const).map(lang => {
            ...
            return (
              ...
              >
                ...
                {lang === 'en'
                  ? t('seller.products.formFieldLangEn')
                  : t('seller.products.formFieldLangNe')}
              </button>
            )
          })}
        </div>
        ...
        <textarea
          ...
          placeholder={
            descLang === 'en'
              ...
              : 'उत्पादन विस्तृत वर्णन गर्नुहोस्...'
          }
          rows={6}
          aria-label={
            descLang === 'en'
      ...
      <Field
        label={t('seller.products.formFieldSpecs')}
        hint={t('seller.products.formFieldSpecsHint')}
      >
        <div className="flex flex-col gap-2">
            ...
            {form.specRows.map(row => (
              ...
              >
                ...
                <button
                  ...
                  aria-label={t('seller.products.formFieldSpecsRemoveAria', {
                    key: row.key || t('seller.products.formFieldSpecsKeyPlaceholder'),
                  })}
          ...
          {form.specRows.length === 0 && (
            <p className="text-[12px] text-text-muted">
              {t('seller.products.formFieldSpecsEmpty')}
            </p>
          )}
          {specsPrefilled && form.specRows.length > 0 && (
            <p className="text-[12px] text-success">
              {t('seller.products.formFieldSpecsPrefilled')}
            </p>
          )}
        </div>
      </Field>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Field
          label={t('seller.products.formFieldHandlingTime')}
          hint={t('seller.products.formFieldHandlingTimeHint')}
        >
          <select
        ...
        <Field
          label={t('seller.products.formFieldReturnPolicy')}
          hint={t('seller.products.formFieldReturnPolicyHint')}
        >
          <select
      ...
      {/* Pickup location */}
      <Field
        label={t('seller.products.formFieldPickupLocation')}
        hint={t('seller.products.formFieldPickupLocationHint')}
      >
        <input
          type="text"
    ...
    <div className="bg-surface rounded-xl border border-border-light overflow-hidden shadow-sm max-h-[calc(100vh-120px)] overflow-y-auto">
      ...
      <div className="p-4 space-y-2">
        ...
        {product.description && (
          <div className="pt-2 border-t border-border-light">
            <p className="text-[12px] text-text-muted line-clamp-3">
      ...
      {hasSpecs && (
        <div className="px-4 pb-4 border-t border-border-light pt-3">
          <p className="text-[12px] font-semibold text-text-muted uppercase mb-2">
            {t('seller.products.formFieldSpecs')}
          </p>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={
          reduced ? { duration: 0 } : { type: 'spring', stiffness: 300, damping: 25, mass: 0.8 }
        }
