...
export default function ReviewStepClient() {
  ...
  if (submitState === 'review' || submitState === 'approved') {
    return (
      <StatusScreen state={submitState} reduced={reduced} onContinue={handleGoToDashboard} t={t} />
    )
  }
      ...
      <div className="flex-1 max-w-[600px] mx-auto w-full px-6">
        ...
        <div className="flex flex-col gap-4">
          <ReviewSection
            ...
            onEdit={() => router.push('/setup-store')}
            t={t}
          >
          ...
          <ReviewSection
            ...
            onEdit={() => router.push('/setup-business')}
            t={t}
          >
      ...
      <div className="sticky bottom-0 bg-background border-t border-border-light px-6 py-3">
        <div className="max-w-[600px] mx-auto flex gap-2">
          ...
          <button
            ...
            className="flex-1 h-[52px] rounded-md bg-primary text-white text-base font-semibold hover:bg-primary-dark transition-colors disabled:opacity-70 flex items-center justify-center gap-2"
            aria-label={
          ...
          >
            {submitState === 'submitting' ? (
              <div className="flex items-center gap-2">
                ...
                <span>{t('seller.setup.submitting')}</span>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <span>{t('seller.setup.submit')}</span>
                <Check size={18} />
              </div>
            )}
          </button>
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6">
      ...
      >
        <motion.div
          ...
          animate={{ scale: 1 }}
          transition={
            reduced ? { duration: 0 } : { type: 'spring', damping: 12, stiffness: 300, delay: 0.2 }
          }
        ...
        <div
          className={`px-3 py-1 rounded-full ${isApproved ? 'bg-success-light' : 'bg-warning-light'}`}
        >
          <span className={`text-xs font-semibold ${isApproved ? 'text-success' : 'text-warning'}`}>
        ...
        {!isApproved && (
          <div className="flex items-center gap-1.5 bg-surface rounded-full px-3 py-1.5 border border-border-light">
            ...
            <span className="text-[13px] font-semibold text-warning">
              {t('seller.setup.underReviewTimeframe')}
            </span>
          </div>
        )}
      ...
      {isApproved && (
        <div className="mt-8">
          <button
            onClick={onContinue}
            className="h-[52px] px-8 rounded-md bg-primary text-white text-base font-semibold hover:bg-primary-dark transition-colors"
          >
            {t('seller.setup.goToDashboard')}
          </button>
        </div>
      )}
    </div>
  ...
  return (
    <div className="bg-surface rounded-lg border border-border-light p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        >
          <Pencil size={13} />
          <span className="text-[13px] font-semibold">{t('seller.setup.editSection')}</span>
        </button>
      </div>
